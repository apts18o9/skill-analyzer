...
const ProfilePage = () => {
  ...
  useEffect(() => {
    // Protect profile from unauthorized user
    if (!loading && !user) {
      navigate('/login');
    }
    ...
    if (user) {
      setProfileData({
        ...
        desiredRoles: (user.desiredRoles || []).join(', ')
      });
    }
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      await updateProfile({
        currentSkills: profileData.currentSkills.split(',').map(item => item.trim()).filter(item => item),
        desiredRoles: profileData.desiredRoles.split(',').map(item => item.trim()).filter(item => item)
      });
